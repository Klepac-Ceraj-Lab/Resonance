...
function load_knead(dir=ENV["ANALYSIS_FILES"]; overwrite=false)
    if isfile(datafiles("read_counts.csv")) && !overwrite
        return CSV.read(datafiles("read_counts.csv"), DataFrame)
    else
        run(Cmd([
        ...
        df = CSV.read(joinpath(ENV["ANALYSIS_FILES"], "kneaddata", "kneaddata_read_counts.tsv"), DataFrame; missingstring="NA")
