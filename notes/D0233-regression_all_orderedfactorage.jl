...
function get_age_factor(cogscorevector::Vector{Float64}, min=0, max=24, by=6)
    intervals = collect(min:by:max)[2:end]
    quantile_class = map(x -> findfirst(x .<= intervals), cogscorevector)
...
X = hcat(
    coerce(get_age_factor(prediction_df.ageMonths), OrderedFactor),
    prediction_df[:, 6:end]
)
