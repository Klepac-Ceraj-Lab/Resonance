...
X = prediction_df_0_6[:, 6:end]
y = prediction_df_0_6[:, 4]
...
# train_y_hat = MLJ.predict(selected_machine, X[train, :])
# train_mae = mean(MLJ.MLJBase.l1(train_y_hat, y[train]))
# mape(train_y_hat, y[train])
# train_cor = Statistics.cor(train_y_hat, y[train])

# test_y_hat = MLJ.predict(selected_machine, X[test, :])
# test_mae = mean(MLJ.MLJBase.l1(test_y_hat, y[test]))
# mape(test_y_hat, y[test])
# test_cor = Statistics.cor(test_y_hat, y[test])
...
ax = Axis(train_set_plot[1, 1],
    title = "Comparison of predicted vs ground truth *Current cogScore* for ages 0-6 months, *TRAIN* set",
    xlabel = "Ground Truth",
    ylabel = "Prediction"
)
...
title = "Comparison of predicted vs ground truth *Current cogScore* for ages 0-6 months, *TEST* set",
    xlabel = "Ground Truth",
    ylabel = "Prediction"
)
...
ax = Axis(all_set_plot[1, 1],
    title = "Comparison of predicted vs ground truth *Current cogScore* for ages 0-6 months, *FULL SAMPLE SET*",
    xlabel = "Ground Truth",
    ylabel = "Prediction"
)
