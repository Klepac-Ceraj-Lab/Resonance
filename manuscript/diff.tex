...
%DIF ADD manuscript/main.tex         Tue Oct 10 14:11:05 2023
...
including \emph{\DIFdelbegin \DIFdel{Gordonibacter pamelae}\DIFdelend \DIFaddbegin \DIFadd{Alistipes obesi}\DIFaddend } and \emph{Blautia wexlerae}, were \DIFdelbegin \DIFdel{significantly }\DIFdelend %DIF >  check RF models for direction of B. wex
associated with higher cognitive function, while some
species such as \emph{Ruminococcus gnavus} were more commonly found in %DIF >  check RF models for direction of R. gnav
...
For example, \emph{B. wexlerae} %DIF >  is this still true?
...
the phylum \textit{Bacteroidetes}, including GABA-producing %DIF >  is this still true?
...
(acetate synthesis II, }\DIFaddend E.S. = \DIFdelbegin \DIFdel{-0.674}\DIFdelend \DIFaddbegin \DIFadd{-0.409}\DIFaddend , q = \DIFdelbegin \DIFdel{0.041). Interestingly, genes
...
higher cognitive function scores \DIFaddbegin \DIFadd{in this age group }\DIFaddend (glutamate synthesis \DIFdelbegin \DIFdel{I}\DIFdelend \DIFaddbegin \DIFadd{II}\DIFaddend , E.S. = \DIFdelbegin \DIFdel{0.242}\DIFdelend \DIFaddbegin \DIFadd{0.154}\DIFaddend , 
q = \DIFdelbegin \DIFdel{0.047; tryptophan synthesis, E.S. = 0.119, q = 0.041}\DIFdelend \DIFaddbegin \DIFadd{0.178}\DIFaddend ). Taken together, these results suggest that
...
\DIFaddbegin \begin{table}[!h]
    ...
    \begin{tabular}{|r|r|r|r|r}
        \hline
        \textbf{\DIFaddFL{Microbial feature}} & \textbf{\DIFaddFL{Demo.}} & \textbf{\DIFaddFL{Test set correlation (± C.I.)}} & \textbf{\DIFaddFL{Test set Root-mean-square error (± C.I.)}} \\\hline
...
\DIFaddend As with LMs, RF models for children under 6 months old
...
\DIFadd{was just outside our cutoff (64\% cumulative importance).
...
could predict }\textit{\DIFadd{future}} \DIFadd{cognitive performance (Figure 3B, D).
Models were consistently able to learn microbial species that are associated
with cognitive performance at a future visit
(mean time between visits: 21.5 months }[\DIFadd{sd = 9.9 months}] \DIFadd{(Figure 3C),
...
}\DIFdelend \DIFaddbegin \DIFadd{Nearly all important taxa in RF models that predicted cognitive function
...
18--120 months, rank = 7)}\DIFdelend \DIFaddbegin \DIFadd{months were also important in
...
}\textit{\DIFadd{K. pneumoniae}} \DIFaddend and \DIFdelbegin \emph{\DIFdel{G. pamelaeae}} %DIFAUXCMD
\DIFdel{(0--6 months , rank =
16; 18--120 months, rank = 20),
while others such as }\emph{\DIFdel{Allistipes
finegoldii}} %DIFAUXCMD
\DIFdel{were age-group specific.
A number of taxa that were important }\DIFdelend \DIFaddbegin \textit{\DIFadd{K. varicola}} \DIFadd{(Figure 3D, E).
...
However, these taxa often had small nominal
p-values. For example, }\emph{\DIFdel{Erysipelatoclostridium ramosum}} %DIFAUXCMD
...
\DIFaddbegin \subsubsection*{\DIFadd{Gut microbial taxa are associated with specific cognitive functions}} %DIF > TODO: add numbers!
...
Mullen Scales of Early Learning (MSEL, birth to 36 months - }\textit{\DIFadd{see Methods}}\DIFadd{). 
...
namely Expressive Language ( test-set RMSE }\hl{XXX}\DIFadd{, test-set correlation }\hl{YYYY}\DIFadd{), 
Gross Motor (test-set RMSE }\hl{XXX}\DIFadd{, test-set correlation }\hl{YYYY}\DIFadd{) and 
Visual Reception (test-set RMSE }\hl{XXX}\DIFadd{, test-set correlation }\hl{YYYY}\DIFadd{).
...
\DIFadd{have similar relative importance across all subscales, others are substantially different.
...
}\textit{\DIFadd{R. faecis}}\DIFadd{, }\textit{\DIFadd{Streptococcus salivarius}} \DIFadd{and }\textit{\DIFadd{F. saccharivorans}} \DIFadd{are
distinctly predictive of GM, and
}\textit{\DIFadd{Clostridium innocuum}}\DIFadd{, }\textit{\DIFadd{B. vulgatus}} 
\DIFadd{stand out on models targeting visual reception.
}
...
\emph{B. vulgatus},
\DIFaddbegin \emph{\DIFadd{B. ovatus}}\DIFadd{,
}\DIFaddend \emph{Fusicatenibacter saccharivorans},
\emph{Ruminococcus torques},
\emph{Eubacterium rectale},
\emph{Coprococcus comes}\DIFaddbegin \DIFadd{,
}\emph{\DIFadd{Asaccharobacter celatus}} \DIFaddend and
...
the right anterior cingulate (respectively, \DIFdelbegin \DIFdel{3.0\% and 2.6\% }\DIFdelend \DIFaddbegin \hl{3.0\% and 2.6\%} \DIFaddend importances).
...
\DIFadd{which }\DIFaddend is heavily associated with the left \DIFdelbegin \DIFdel{pars opercularis (2.7\% }\DIFdelend \DIFaddbegin \DIFadd{precentral area (}\hl{2.7\%} \DIFaddend importance). 
...
is the highest for a microbe in RF models (\DIFdelbegin \DIFdel{4.0\% }\DIFdelend \DIFaddbegin \hl{4.0\%} \DIFaddend importance);
...
\DIFdelbegin \DIFdel{Left = 2.3\%, Right = 2.8\%}\DIFdelend \DIFaddbegin \hl{Left = 2.3\%, Right = 2.8\%}\DIFaddend ), along with upper regions like the left
precentral (2.3\% importance) and paracentral lobes (importances, \DIFdelbegin \DIFdel{Left = 2.6\%, Right = 1.9\%}\DIFdelend \DIFaddbegin \hl{Left = 2.6\%, Right = 1.9\%}\DIFaddend ).
...
pathobiontic }\textit{\DIFadd{Enterobacteriaceae}} \DIFadd{such as
...
changes to neural responses in some types of stimulae, notably visual\mbox{%DIFAUXCMD
...
butyrate-producing }\textit{\DIFadd{Fusicatenibacter saccharivorans}} \DIFadd{is among the 15\% most important predictors
