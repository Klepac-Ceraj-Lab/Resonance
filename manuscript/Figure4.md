...
E_Subfig  = GridLayout(figure[3,1], alignmode=Outside())
```
...
```julia
panelA_plot_df = vcat(
    insertcols(DataFrames.combine(groupby(regression_currentCogScores_18to120mo_onlydemo.merits, :Hyperpar_Idx), :Test_Cor => (x -> round(mean(x); digits = 2)) => :Test_Cor_mean), 1, :xs => 1, :grp => 1,:color => :black ),
    insertcols(DataFrames.combine(groupby(regression_currentCogScores_18to120mo_demoplustaxa.merits, :Hyperpar_Idx), :Test_Cor => (x -> round(mean(x); digits = 2)) => :Test_Cor_mean), 1, :xs => 2, :grp => 1,:color => :black ),
    insertcols(DataFrames.combine(groupby(regression_currentCogScores_18to120mo_demoplusecs.merits, :Hyperpar_Idx), :Test_Cor => (x -> round(mean(x); digits = 2)) => :Test_Cor_mean), 1, :xs => 3, :grp => 1,:color => :black ),
    insertcols(DataFrames.combine(groupby(regression_currentExpressiveLanguages_18to120mo_onlydemo.merits, :Hyperpar_Idx), :Test_Cor => (x -> round(mean(x); digits = 2)) => :Test_Cor_mean), 1, :xs => 1, :grp => 2,:color => :gray25 ),
    insertcols(DataFrames.combine(groupby(regression_currentExpressiveLanguages_18to120mo_demoplustaxa.merits, :Hyperpar_Idx), :Test_Cor => (x -> round(mean(x); digits = 2)) => :Test_Cor_mean), 1, :xs => 2, :grp => 2,:color => :gray25 ),
    insertcols(DataFrames.combine(groupby(regression_currentExpressiveLanguages_18to120mo_demoplusecs.merits, :Hyperpar_Idx), :Test_Cor => (x -> round(mean(x); digits = 2)) => :Test_Cor_mean), 1, :xs => 3, :grp => 2,:color => :gray25 ),
    insertcols(DataFrames.combine(groupby(regression_currentGrossMotors_18to120mo_onlydemo.merits, :Hyperpar_Idx), :Test_Cor => (x -> round(mean(x); digits = 2)) => :Test_Cor_mean), 1, :xs => 1, :grp => 3,:color => :gray50 ),
    insertcols(DataFrames.combine(groupby(regression_currentGrossMotors_18to120mo_demoplustaxa.merits, :Hyperpar_Idx), :Test_Cor => (x -> round(mean(x); digits = 2)) => :Test_Cor_mean), 1, :xs => 2, :grp => 3,:color => :gray50 ),
    insertcols(DataFrames.combine(groupby(regression_currentGrossMotors_18to120mo_demoplusecs.merits, :Hyperpar_Idx), :Test_Cor => (x -> round(mean(x); digits = 2)) => :Test_Cor_mean), 1, :xs => 3, :grp => 3,:color => :gray50 ),
    insertcols(DataFrames.combine(groupby(regression_currentVisualReceptions_18to120mo_onlydemo.merits, :Hyperpar_Idx), :Test_Cor => (x -> round(mean(x); digits = 2)) => :Test_Cor_mean), 1, :xs => 1, :grp => 4,:color => :gray75 ),
    insertcols(DataFrames.combine(groupby(regression_currentVisualReceptions_18to120mo_demoplustaxa.merits, :Hyperpar_Idx), :Test_Cor => (x -> round(mean(x); digits = 2)) => :Test_Cor_mean), 1, :xs => 2, :grp => 4,:color => :gray75 ),
    insertcols(DataFrames.combine(groupby(regression_currentVisualReceptions_18to120mo_demoplusecs.merits, :Hyperpar_Idx), :Test_Cor => (x -> round(mean(x); digits = 2)) => :Test_Cor_mean), 1, :xs => 3, :grp => 4,:color => :gray75 ),
)
```
...
```julia
composite_importances = rename(weighted_hpimportances(regression_currentCogScores_18to120mo_demoplustaxa; normalize_importances=true), :weightedImportance => :Composite)
el_importances = rename(weighted_hpimportances(regression_currentExpressiveLanguages_18to120mo_demoplustaxa; normalize_importances=true), :weightedImportance => :ExpressiveLanguage)
gm_importances = rename(weighted_hpimportances(regression_currentGrossMotors_18to120mo_demoplustaxa; normalize_importances=true), :weightedImportance => :GrossMotor)
vr_importances = rename(weighted_hpimportances(regression_currentVisualReceptions_18to120mo_demoplustaxa; normalize_importances=true), :weightedImportance => :VisualReception)
...
panelB_plot_df = @chain vcat(
    insertcols(weighted_hpimportances(regression_currentCogScores_18to120mo_demoplustaxa; normalize_importances=true), 1, :grp => 1, :color => "black"),
    insertcols(weighted_hpimportances(regression_currentExpressiveLanguages_18to120mo_demoplustaxa; normalize_importances=true), 1, :grp => 2, :color => "gray25"),
    insertcols(weighted_hpimportances(regression_currentGrossMotors_18to120mo_demoplustaxa; normalize_importances=true), 1, :grp => 3, :color => "gray50"),
    insertcols(weighted_hpimportances(regression_currentVisualReceptions_18to120mo_demoplustaxa; normalize_importances=true), 1, :grp => 4, :color => "gray75")
) begin
...
Legend(AB_Subfig[1,3],
    [
        MarkerElement(; marker=:rect, color="black"),
        MarkerElement(; marker=:rect, color="gray25"),
        MarkerElement(; marker=:rect, color="gray50"),
        MarkerElement(; marker=:rect, color="gray75")
    ],
...
CairoMakie.xlims!(axE, [0, nrow(weighted_noage_importances)+1])
...
for bug in highlight_bugs
    ...
    poly!(axE, Point2f[(i-0.5, 0), (i+0.5, 0), (i+0.5, maximp), (i-0.5, maximp)]; color=hlbugs_color[bug])
end
CairoMakie.scatter!(axE, xs .+ rand(Normal(0, 0.1), length(xs)), ys;)
...
Label(CD_Subfig[1, 1, TopLeft()], "C", fontsize = 26,font = :bold, padding = (0, 240, 5, 0), halign = :right)
Label(CD_Subfig[1, 2, TopLeft()], "D", fontsize = 26,font = :bold, padding = (0, 200, 5, 0), halign = :right)
