...
# Importance Heatmaps
######
nbugs_toplot = length(interesting_taxa) # the top 1/3 bugs (out of 129) From intersecting the top mean importances and top max importances, account for average of 50% importance on segments
...
maximp = findmax(Matrix(weighted_noage_importances[!, 2:end]))[1]

for (col, bug) in zip(ColorSchemes.Set3_12, highlight_bugs)
...
Legend(C_subfig[1,2], [MarkerElement(; marker=:rect, color = ColorSchemes.Set3_12[i]) for i in 1:length(highlight_bugs)], highlight_bugs)
