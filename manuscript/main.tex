...
\author{%
    \parbox{\linewidth}{\centering
        ...
        Fatoumata Barry,$^{2}$
        Rosa Cano Lorente,$^{2}$
        The RESONANCE Consortium$^{1\dagger}$,
        Curtis Huttenhower,$^{4-7}$
        Muriel Bruchhage,$^{3}$
        Viren D'Sa,$^{2}$
        Sean Deoni,$^{2}$
        Vanja Klepac-Ceraj$^{\ast}$
    }
\\
...
\normalsize{$^{5}$Department of Immunology and Infectious Diseases, Harvard T.H. Chan School of Public Health, Boston, MA, USA}\\
\normalsize{$^{6}$Harvard Chan Microbiome in Public Health Center, Harvard T.H. Chan School of Public Health, Boston, MA, USA}\\
\normalsize{$^{7}$Associate Member, Broad Institute of MIT and Harvard}\\
\\
\normalsize{$^\dagger$The RESONANCE consortium authors and collaborators and affiliations are listed in the supplementary materials.}\\
...
\begin{abstract}
    Emerging evidence implicates gut microbial metabolism in neurodevelopmental disorders,
    but its influence on typical neurodevelopment has not been explored in detail.
    We investigated the relationship between the microbiome and neuroanatomy and cognition of 381 healthy children,
    demonstrating that differences in microbial taxa and genes are associated
    with overall cognitive function and the size of brain regions.
    Using a combination of statistical and machine learning models,
    we showed that species including \textit{Alistipes obesi}, \textit{Blautia wexlerae}, and \textit{Ruminococcus gnavus}
    were enriched or depleted in children with higher cognitive function scores.
    Microbial metabolism of short-chain fatty acids was also associated with cognitive function.
    In addition, machine models were able to predict the volume of brain regions from microbial profiles,
    and taxa that were important in predicting cognitive function
    were also important for predicting individual brain regions and specific subscales of cognitive function.
    These findings provide potential biomarkers of neurocognitive development
    and may enable development of targets for early detection and intervention.
\end{abstract}

\subsubsection*{Short title}

Gut microbes are linked to child neurocognition

\subsubsection*{Teaser}

The complex community of microorganisms living in the guts of infants and children
...
\subsubsection*{The RESONANCE cohort is part of a study of child development}
...
and the WISC-V for children older than 6), but were normalized to a common scale (fig. 1D),
As expected, the greatest differences in microbial taxa were observed across
child age (fig. 1B, E; PCoA axis 1), with older children primarily
stratified into Bacteroidetes-dominant, Firmicutes-dominant, or
high-abundance \emph{Prevotella copri} (fig. S2).
Overall subject-to-subject variation in gut microbial genes
and brain volume profiles was similarly driven largely by subject age (fig. 1C, F).
...
\cite{liuAlteredGutMicrobiota2019,wanUnderdevelopmentGutMicrobiota2021,magnussonRelationshipsDietrelatedChanges2015,mayneris-perxachsMicrobiotaAlterationsProline2022,needhamGutderivedMetaboliteAlters2022},
...
in gut microbiomes in children over 18 months old (fig. 1G).
...
(fig. 1G, R\textsuperscript{2} = 0.0099, q \textless{} 0.01),
...
taxonomic diversity and broad lack of overlap between taxa in infants (fig. 1G).
...
(fig. 1H, p \textless{} 0.001). Other functional
...
prevalence in a given age group (fig. 2A, N\textsubscript{species} = 116 for 0--120 months, 
...
in children with higher cognitive function scores (fig. 2B),
...
(fig. 2B). \textit{Sutterella wadsworthensis} was the only microbial species
...
and concurrent cognitive ability score (table 2, fig. 2D-F) and found that
...
\begin{table}[!h]
    ...
    \end{tabular}
    \caption{\label{tab:rfbench} \textbf{RF models on microbial taxa and genes can learn associations with concurrently measured cognitive performance.}
    Benchmark metrics for the cognitive assessment score
...
significant when testing the relationship with LMs (fig.
...
(table 3, fig. 3B, E). Subject age was consistently ranked highly in
...
could predict \textit{future} cognitive performance measured at least 6 months
...
(mean time between visits: 21.5 months, S.D. = 9.9 months; fig. 3G,
...
\begin{table}[!h]
    ...
    \end{tabular}
    \caption{\label{tab:rfbench2}  \textbf{RF models on microbial taxa and genes can learn associations with future cognitive performance.}
...
\textit{K. pneumoniae} and \textit{K. varicola} (fig. 3C, F).
...
stand out on models targeting visual reception (tables S2-4).
...
\begin{table}[!h]
    \begin{center}
        ...
        \end{tabular}
        \caption{\label{tab:subscalebenchmark}  \textbf{RF models on microbial taxa and genes can learn associations with performance in cognitive subdomains.}
        Benchmark metrics for the Mullen subscales prediction models.
        Confidence intervals (C.I.) are calculated from the
        distribution of metrics from repeated CV at a confidence level of 95.
        Root mean square error (RMSE) was used to estimate how well the random forest model was able to predict the outcome.
        Demo. column indicates whether demographic factors such as maternal education, sex and age were included as predictors.
...
predicted by RF models trained on microbial taxa (tables 5, 6, and S6), in particular those that were highly correlated with age.
...
\begin{table}[!h]
    ...
    \end{tabular}
    \caption{\label{tab:neuro}  \textbf{RF models on microbial taxa predicting brain regions have generally good performance.}
...
regardless of model performance (fig. 4C-E, table S2).
...
pathobionts in the family \textit{Enterobacteriaceae} such as
...
associated with cognitive function (fig. 3C, F). This kind of
...
are associated with cognitive function scores (fig. 2). 
...
metagenomes.
Statistical analyses, including linear regression
and feature set enrichment analysis were performed
using the julia programming language and third-party packages
\cite{bezansonJuliaFreshApproach2017,bonhamMicrobiomeJlBiobakeryUtils2021,danischMakieJlFlexible2021,blaomMLJJuliaPackage2020,ben_sadeghi_2022_7359268,dahua_lin_2023_7695673,douglas_bates_2023_7734970}.
...
datasets \cite{woodruffChronicInflammationNeutrophil2023a}.
...
\begin{itemize}[noitemsep]
    \item Conceptualization: KSB, VKC, CH, and SD
    \item Methodology: KSB, VKC, CH, and SD
    \item Project administraion: KSB, VKC, CH, and SD
    \item Funding acquisition: VKC, SD, and VD
    \item Data curation: KSB, GFB, SHM, MB, FB, RCL, and JB
    \item Formal analyses: KSB, GFB, and MB
    \item Investigation: KSB, GFB, SHM, EW, FB, RCL, and MB
    \item Resources: SHM, EW, FB, RCL, JB, CH, and MB
    \item Software and data visualizations: KSB and GFB
    \item Supervision: KSB, MB, SD, VD, and VKC 
    \item Validation: KSB, SHM, JB, and MB
    \item Initial draft: KSB and GFB
    \item Revision and editing: KSB, GFB, SD, CH, and VKC
\end{itemize}

Members of the RESONANCE consortium also contributed to
project planning, conceptualization, and funding acquisition.
...
and 3rd party packages (see Materials and Methods).
...
download of input files are available on GitHub at https://github.com/Klepac-Ceraj-Lab/Resonance,
and archived on Zenodo \cite{kevin_bonham_2023_7647510}.
...
\begin{figure}[!h]
    ...
    \caption{
        \label{fig:1} \textbf{The RESONANCE cohort is a diverse cohort with accelerated longitudinal sampling.} (\textbf{A}) Stool samples,
...
\begin{figure}[!htb]
    ...
    \caption{
        \textbf{Taxa and gene functional groups are associated with cognitive function.}
...
\begin{figure}[!htb]
    ...
    \caption{
        \textbf{Random forest models predict both concurrent and future measured cognitive function.}
...
\begin{figure}[!htb]
    ...
    \caption{
        \textbf{Coupled importance of microbes in neuroanatomy and subscores are consistent with known subcortical associations with cognition.}
